...
После соединения с сервером прибод должен себя идентифицировать - передать свой IMEI. Пока это не произведено, никакие данные от прибора не должны быть приняты - сервер обязан возвращать ошибку.
...
| Значение | Описание |
|:---:|:---|
| 0 | *Регистрационные данные.* IMEI, номер телефона, и т.п.
...
Для экономии GPRS-трафика, прибор может уплотнять передаваемые данные.
При этом первый блок данных идет в развернутом виде (0xF1 или 0xF2).

| Размер | Описание |
|:---:|:---|
| 8 бит | Дельта времени (+0...+255 секунд)
| 8 бит | Дельта широты (-128...+127) единиц
| 8 бит | Дельта долготы (-128...+127) единиц
| 16 бит | Скорость
| 8 бит | Направление
| 4 бит | Дельта спутников (-8...+7)
| 4 бит | Дельта напряжения основного аккумулятора (-8/100...+7/100 В)
| 4 бит | Дельта напряжения резервного аккумулятора (-8/100...+7/100 В)
| 8 бит | Тип точки (причина фиксации точки)
| 8 бит | Контрольная сумма записи (простая сумма всех значений).
