...
MapModule.controller "mapControl", [
    ...
    ($scope, $rootScope, Systems) ->
        ...
        # Systems.systems
        $scope.name ='misko'
        log 'mapControl:Scope=', $scope, Systems, $scope.systems
        ###
...
MapModule.directive 'map', () ->
    {
        ...
        link: ($scope, element, attrs, controller) ->
            ...
            ###
            $scope.lastmarkers = new LastMarkers(window.config.map)
            $scope.$watch( "systems", (nv, ov) ->
                #console.log '**** systems > changed:', nv, ov

                for k, v of nv.systems
                    if v.last.point
                        #console.log 'v=', v.last.point
                        $scope.lastmarkers.setMarker(k, v.last.point)
                        #if not $scope.lastmarkers.markers[k]
